...
struct BubbleSort_vtable * BubbleSort_vtable_ptr;
void* get_BubbleSort_vtable_();
struct BBS_vtable BBS_vtable_;
struct BBS_vtable * BBS_vtable_ptr;
void* get_BBS_vtable_();
...
// vtable get methods
void* get_BubbleSort_vtable_(){
  if(BubbleSort_vtable_ptr== NULL){
    struct BubbleSort_vtable temp = 
      {
      };
    memcpy(&BubbleSort_vtable_, &temp, sizeof(temp));
    int equal = (int)memcmp(&BubbleSort_vtable_, &temp, sizeof(temp));
    if(equal != 0){
      printf("Memory copy error: buffer polluted\n");
      exit(-2);
    }
    BubbleSort_vtable_ptr = &BubbleSort_vtable_;
  }
  return (void*)(BubbleSort_vtable_ptr);
}

void* get_BBS_vtable_(){
  if(BBS_vtable_ptr== NULL){
    struct BBS_vtable temp = 
      {
        BBS_Start,
        BBS_Sort,
        BBS_Print,
    ...
    memcpy(&BBS_vtable_, &temp, sizeof(temp));
    int equal = (int)memcmp(&BBS_vtable_, &temp, sizeof(temp));
    if(equal != 0){
      printf("Memory copy error: buffer polluted\n");
      exit(-2);
    }
    BBS_vtable_ptr = &BBS_vtable_;
  }
  return (void*)(BBS_vtable_ptr);
}
...
{
  struct BBS * x_0;
  System_out_println ((x_0=((struct BBS*)(Tiger_new (get_BBS_vtable_(), sizeof(struct BBS)))), x_0->vptr->Start(x_0, 10)));
  return 0;
}
