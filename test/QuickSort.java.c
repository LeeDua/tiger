// This is automatically generated by the Tiger compiler.
// Do NOT modify!
#include <stdio.h>
#include <stdlib.h>
#include <string.h>

void *Tiger_new (void *vtable, int size);
int System_out_println (int i);

// structures
struct QuickSort
{
  struct QuickSort_vtable *vptr;
};
struct QS
{
  struct QS_vtable *vptr;
  int* number;
  int size;
};
// vtables structures
struct QuickSort_vtable
{
};

struct QS_vtable
{
  int (*Start)();
  int (*Sort)();
  int (*Print)();
  int (*Init)();
};


// vtable ptr and init method decls
struct QuickSort_vtable QuickSort_vtable_;
struct QuickSort_vtable * QuickSort_vtable_ptr;
void* get_QuickSort_vtable_();
struct QS_vtable QS_vtable_;
struct QS_vtable * QS_vtable_ptr;
void* get_QS_vtable_();

// methods
int QS_Start(struct QS * this, int sz)
{
  int aux01;
  struct QS * x_1;
  struct QS * x_2;
  struct QS * x_3;
  struct QS * x_4;

  aux01 = (x_1=this, x_1->vptr->Init(x_1, sz));
  aux01 = (x_2=this, x_2->vptr->Print(x_2));
  System_out_println (9999);
  aux01 = this->size - 1;
  aux01 = (x_3=this, x_3->vptr->Sort(x_3, 0, aux01));
  aux01 = (x_4=this, x_4->vptr->Print(x_4));
  return 0;
}
int QS_Sort(struct QS * this, int left, int right)
{
  int v;
  int i;
  int j;
  int nt;
  int t;
  int cont01;
  int cont02;
  int aux03;
  struct QS * x_5;
  struct QS * x_6;

  t = 0;
  if (left < right)
    {
      v = this->number[right];
      i = left - 1;
      j = right;
      cont01 = 1;
      while(cont01)
        {
          cont02 = 1;
          while(cont02)
            {
              i = i + 1;
              aux03 = this->number[i];
              if (!(aux03 < v))
                cont02 = 0;
              else
                cont02 = 1;
            }
          cont02 = 1;
          while(cont02)
            {
              j = j - 1;
              aux03 = this->number[j];
              if (!(v < aux03))
                cont02 = 0;
              else
                cont02 = 1;
            }
          t = this->number[i];
          this->number[i] = this->number[j];
          this->number[j] = t;
          if (j < i + 1)
            {
              cont01 = 0;
            }
          else
            {
              cont01 = 1;
            }
        }
      this->number[j] = this->number[i];
      this->number[i] = this->number[right];
      this->number[right] = t;
      nt = (x_5=this, x_5->vptr->Sort(x_5, left, i - 1));
      nt = (x_6=this, x_6->vptr->Sort(x_6, i + 1, right));
    }
  else
    nt = 0;
  return 0;
}
int QS_Print(struct QS * this)
{
  int j;

  j = 0;
  while(j < this->size)
    {
      System_out_println (this->number[j]);
      j = j + 1;
    }
  return 0;
}
int QS_Init(struct QS * this, int sz)
{

  this->size = sz;
  this->number = (int*)malloc(sizeof(int)*sz);
  this->number[0] = 20;
  this->number[1] = 7;
  this->number[2] = 12;
  this->number[3] = 18;
  this->number[4] = 2;
  this->number[5] = 11;
  this->number[6] = 6;
  this->number[7] = 9;
  this->number[8] = 19;
  this->number[9] = 5;
  return 0;
}

// vtable get methods
void* get_QuickSort_vtable_(){
  if(QuickSort_vtable_ptr== NULL){
    struct QuickSort_vtable temp = 
      {
      };
    memcpy(&QuickSort_vtable_, &temp, sizeof(temp));
    int equal = (int)memcmp(&QuickSort_vtable_, &temp, sizeof(temp));
    if(equal != 0){
      printf("Memory copy error: buffer polluted\n");
      exit(-2);
    }
    QuickSort_vtable_ptr = &QuickSort_vtable_;
  }
  return (void*)(QuickSort_vtable_ptr);
}

void* get_QS_vtable_(){
  if(QS_vtable_ptr== NULL){
    struct QS_vtable temp = 
      {
        QS_Start,
        QS_Sort,
        QS_Print,
        QS_Init,
      };
    memcpy(&QS_vtable_, &temp, sizeof(temp));
    int equal = (int)memcmp(&QS_vtable_, &temp, sizeof(temp));
    if(equal != 0){
      printf("Memory copy error: buffer polluted\n");
      exit(-2);
    }
    QS_vtable_ptr = &QS_vtable_;
  }
  return (void*)(QS_vtable_ptr);
}


// main method
int Tiger_main ()
{
  struct QS * x_0;
  System_out_println ((x_0=((struct QS*)(Tiger_new (get_QS_vtable_(), sizeof(struct QS)))), x_0->vptr->Start(x_0, 10)));
  return 0;
}




