...
struct BinarySearch_vtable * BinarySearch_vtable_ptr;
void* get_BinarySearch_vtable_();
struct BS_vtable BS_vtable_;
struct BS_vtable * BS_vtable_ptr;
void* get_BS_vtable_();
...
// vtable get methods
void* get_BinarySearch_vtable_(){
  if(BinarySearch_vtable_ptr== NULL){
    struct BinarySearch_vtable temp = 
      {
      };
    memcpy(&BinarySearch_vtable_, &temp, sizeof(temp));
    int equal = (int)memcmp(&BinarySearch_vtable_, &temp, sizeof(temp));
    if(equal != 0){
      printf("Memory copy error: buffer polluted\n");
      exit(-2);
    }
    BinarySearch_vtable_ptr = &BinarySearch_vtable_;
  }
  return (void*)(BinarySearch_vtable_ptr);
}

void* get_BS_vtable_(){
  if(BS_vtable_ptr== NULL){
    struct BS_vtable temp = 
      {
        BS_Start,
        BS_Search,
        BS_Div,
    ...
    memcpy(&BS_vtable_, &temp, sizeof(temp));
    int equal = (int)memcmp(&BS_vtable_, &temp, sizeof(temp));
    if(equal != 0){
      printf("Memory copy error: buffer polluted\n");
      exit(-2);
    }
    BS_vtable_ptr = &BS_vtable_;
  }
  return (void*)(BS_vtable_ptr);
}
...
{
  struct BS * x_0;
  System_out_println ((x_0=((struct BS*)(Tiger_new (get_BS_vtable_(), sizeof(struct BS)))), x_0->vptr->Start(x_0, 20)));
  return 0;
}
