...
struct LinearSearch_vtable * LinearSearch_vtable_ptr;
void* get_LinearSearch_vtable_();
struct LS_vtable LS_vtable_;
struct LS_vtable * LS_vtable_ptr;
void* get_LS_vtable_();
...
// vtable get methods
void* get_LinearSearch_vtable_(){
  if(LinearSearch_vtable_ptr== NULL){
    struct LinearSearch_vtable temp = 
      {
      };
    memcpy(&LinearSearch_vtable_, &temp, sizeof(temp));
    int equal = (int)memcmp(&LinearSearch_vtable_, &temp, sizeof(temp));
    if(equal != 0){
      printf("Memory copy error: buffer polluted\n");
      exit(-2);
    }
    LinearSearch_vtable_ptr = &LinearSearch_vtable_;
  }
  return (void*)(LinearSearch_vtable_ptr);
}

void* get_LS_vtable_(){
  if(LS_vtable_ptr== NULL){
    struct LS_vtable temp = 
      {
        LS_Start,
        LS_Print,
        LS_Search,
    ...
    memcpy(&LS_vtable_, &temp, sizeof(temp));
    int equal = (int)memcmp(&LS_vtable_, &temp, sizeof(temp));
    if(equal != 0){
      printf("Memory copy error: buffer polluted\n");
      exit(-2);
    }
    LS_vtable_ptr = &LS_vtable_;
  }
  return (void*)(LS_vtable_ptr);
}
...
{
  struct LS * x_0;
  System_out_println ((x_0=((struct LS*)(Tiger_new (get_LS_vtable_(), sizeof(struct LS)))), x_0->vptr->Start(x_0, 10)));
  return 0;
}
